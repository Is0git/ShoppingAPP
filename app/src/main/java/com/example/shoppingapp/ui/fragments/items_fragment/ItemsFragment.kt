...
class ItemsFragment : DaggerFragment(), ItemListener {
    ...
    override fun onCreateView(
        ...
        itemsViewModel.getItems(args.typeID)
...
//        itemsViewModel.items.observe(viewLifecycleOwner, Observer { adapter.submitList(it) })
    ...
    override fun onItemClick(item: Item, binding: ItemsListBinding) {
        ...
        val extras = FragmentNavigatorExtras(binding.mainCard to transitionName).also {
            binding.mainCard.transitionName = transitionName
        }
        val destination =
            ItemsFragmentDirections.actionItemsFragmentToFullItemFragment(item, transitionName)
